...
async function handleMomentum(message, args, comment) {
  ...
  if (speedMatch && speedMatch[1]) {
    ...
    const multiplier = triggers.blitz ? 10 : 5;
...
async function handleAcceleration(message, args, comment) {
  ...
  if (!validateMinimumRank(message, mrRank, 'D', 'Acceleration', comment)) {
    return;
  }
...
async function handleEngage(message, args, comment) {
  ...
  // Check for triggers
  const triggers = parseTriggers(comment, {
    accretion: /\baccretion\b/i,
    regalia: /\bregalia\b/i
  });

  // Calculate HP cost reduction from Regalia
  const REGALIA_REDUCTION = { e: 0, d: 5, c: 5, b: 10, a: 10, s: 15 };
  const reduction = triggers.regalia ? (REGALIA_REDUCTION[mrRank] ?? 0) : 0;
  ...
  const embed = new EmbedBuilder()
    .setColor(EMBED_COLORS.alter)
    .setAuthor({ name: `${displayName}'s Sub-Action`, iconURL: message.author.displayAvatarURL() })
    .setTitle(triggers.accretion ? 'Engage - Accretion' : 'Engage - Redo')
...
async function handleEmpower(message, args, comment) {
  ...
  if (!validateMinimumRank(message, mrRank, 'A', 'Empower', comment)) {
    return;
  }
...
async function handleMark(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Mark', comment)) {
    return;
  }
...
async function handleHyperInsight(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Hyper Insight', comment)) {
    return;
  }
  ...
  if (triggers.ultra) {
    if (!validateMinimumRank(message, mrRank, 'A', 'Ultra', comment)) {
      return;
    }
  }
  ...
  const finalBreakDamage = triggers.ultra ? baseBreakDamage * 2 : baseBreakDamage;

  // Embed
  const embed = new EmbedBuilder()
    .setColor(EMBED_COLORS.alter)
    .setAuthor({ name: `${displayName}'s Sub-Action`, iconURL: message.author.displayAvatarURL() })
    .setTitle(triggers.ultra ? 'Hyper Insight - Ultra' : 'Hyper Insight')
...
async function handleHyperInstinct(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Hyper Instinct', comment)) {
    return;
  }
  ...
  const finalSaveBonus = triggers.ultra ? baseSaveBonus * 2 : baseSaveBonus;

  // Embed
  const embed = new EmbedBuilder()
    .setColor(EMBED_COLORS.alter)
    .setAuthor({ name: `${displayName}'s Sub-Action`, iconURL: message.author.displayAvatarURL() })
    .setTitle(triggers.ultra ? 'Hyper Instinct - Ultra' : 'Hyper Instinct')
...
async function handleRegenerate(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Regenerate', comment)) {
    return;
  }
  ...
  const embed = new EmbedBuilder()
    .setColor(EMBED_COLORS.alter)
    .setAuthor({ name: `${displayName}'s Sub-Action`, iconURL: message.author.displayAvatarURL() })
    .setTitle(triggers.power ? 'Regenerate - Power' : 'Regenerate')
...
async function handleInfuse(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Infuse', comment)) {
    return;
  }
...
async function handleAdapt(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Adapt', comment)) {
    return;
  }
  ...
  const embed = new EmbedBuilder()
    .setColor(EMBED_COLORS.alter)
    .setAuthor({ name: `${displayName}'s Sub-Action`, iconURL: message.author.displayAvatarURL() })
    .setTitle(triggers.prowl ? 'Adapt - Prowl' : (triggers.fend ? 'Adapt - Fend' : 'Adapt'))
...
async function handleEvolve(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Evolve', comment)) {
    return;
  }
...
async function handleCoordinate(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Coordinate', comment)) {
    return;
  }
...
async function handleCharge(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Charge', comment)) {
    return;
  }

  // Check for mode triggers
  const triggers = parseTriggers(comment, {
    charging: /\bcharging\b/i
  });
  const commentString = typeof comment === 'string' ? comment : '';
  ...
  const embed = new EmbedBuilder()
    .setColor(EMBED_COLORS.alter)
    .setAuthor({ name: `${displayName}'s Sub-Action`, iconURL: message.author.displayAvatarURL() })
    .setTitle(releaseActive ? 'Charge - Release' : (triggers.charging ? 'Charge - Charging' : 'Charge'))
...
async function handleProfane(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Profane', comment)) {
    return;
  }

  // Check for Apostasy mode (requires B rank)
  const triggers = parseTriggers(comment, {
    apostasy: /\bapostasy\b/i
  });

  // Apostasy rank validation
  if (triggers.apostasy && !['b', 'a', 's'].includes(mrRank)) {
    const embed = new EmbedBuilder()
      .setColor(EMBED_COLORS.error)
      .setTitle('Invalid Rank')
...
async function handleRegalia(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Regalia', comment)) {
    return;
  }
...
async function handleAnatomy(message, args, comment) {
  ...
  if (!validateMinimumRank(message, mrRank, 'D', 'Anatomy', comment)) {
    return;
  }
...
async function handleBestowed(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Bestowed', comment)) {
    return;
  }
...
async function handleCombatFocus(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Combat Focus', comment)) {
    return;
  }
...
async function handleUtilityFocus(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Utility Focus', comment)) {
    return;
  }
...
async function handleDefenseFocus(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Defense Focus', comment)) {
    return;
  }
...
async function handleSpeedFocus(message, args, comment) {
  ...
  // Rank validation (minimum D rank)
  if (!validateMinimumRank(message, mrRank, 'D', 'Speed Focus', comment)) {
    return;
  }
...
async function handleRage(message, args, comment) {
  ...
  if (!triggers.frenzy && !hasCapForRank) {
    const embed = new EmbedBuilder()
      .setColor(EMBED_COLORS.error)
      .setTitle('Unavailable MR Rank')
  ...
  const embed = new EmbedBuilder()
    .setColor(EMBED_COLORS.alter)
    .setAuthor({ name: `${displayName}'s Sub-Action`, iconURL: message.author.displayAvatarURL() })
    .setTitle(triggers.frenzy ? '(Alter) Frenzied Rage' : '(Alter) Rage')
...
async function handleGift(message, args, comment) {
  ...
  if (!validateMinimumRank(message, mrRank, 'B', 'Gift', comment)) {
    return;
  }
...
async function handleFollowUp(message, args, comment) {
  ...
  // Rank validation (minimum C rank)
  if (!validateMinimumRank(message, mrRank, 'C', 'Follow-up', comment)) {
    return;
  }
